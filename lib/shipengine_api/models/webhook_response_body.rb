...
module ShipEngineApi
  class WebhookResponseBody
    # A string that uniquely identifies the webhook id
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'webhook_id')
    ...
    def list_invalid_properties
      ...
      if !@webhook_id.nil? && @webhook_id.to_s.length > 25
        invalid_properties.push('invalid value for "webhook_id", the character length must be smaller than or equal to 25.')
      end

      if !@webhook_id.nil? && @webhook_id.to_s.length < 1
        invalid_properties.push('invalid value for "webhook_id", the character length must be great than or equal to 1.')
      end

      if !@webhook_id.nil? && @webhook_id !~ Regexp.new(/^se(-[a-z0-9]+)+$/)
        invalid_properties.push('invalid value for "webhook_id", must conform to the pattern /^se(-[a-z0-9]+)+$/.')
      end
    ...
    def valid?
      return false if !@webhook_id.nil? && @webhook_id.to_s.length > 25
      return false if !@webhook_id.nil? && @webhook_id.to_s.length < 1
      return false if !@webhook_id.nil? && @webhook_id !~ Regexp.new(/^se(-[a-z0-9]+)+$/)
      true
    end
    ...
    def webhook_id=(webhook_id)
      if !webhook_id.nil? && webhook_id.to_s.length > 25
        fail ArgumentError, 'invalid value for "webhook_id", the character length must be smaller than or equal to 25.'
      end

      if !webhook_id.nil? && webhook_id.to_s.length < 1
        fail ArgumentError, 'invalid value for "webhook_id", the character length must be great than or equal to 1.'
      end

      if !webhook_id.nil? && webhook_id !~ Regexp.new(/^se(-[a-z0-9]+)+$/)
        fail ArgumentError, 'invalid value for "webhook_id", must conform to the pattern /^se(-[a-z0-9]+)+$/.'
      end
