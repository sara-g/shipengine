...
module ShipEngineApi
  class UpsInvoice
    ...
    # A string that uniquely identifies the control id
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'control_id')
        self.control_id = attributes[:'control_id']
      end

      if attributes.has_key?(:'invoice_amount')
    ...
    def list_invalid_properties
      ...
      if !@control_id.nil? && @control_id.to_s.length > 25
        invalid_properties.push('invalid value for "control_id", the character length must be smaller than or equal to 25.')
      end

      if !@control_id.nil? && @control_id.to_s.length < 1
        invalid_properties.push('invalid value for "control_id", the character length must be great than or equal to 1.')
      end

      if !@control_id.nil? && @control_id !~ Regexp.new(/^se(-[a-z0-9]+)+$/)
        invalid_properties.push('invalid value for "control_id", must conform to the pattern /^se(-[a-z0-9]+)+$/.')
      end
    ...
    def valid?
      return false if !@control_id.nil? && @control_id.to_s.length > 25
      return false if !@control_id.nil? && @control_id.to_s.length < 1
      return false if !@control_id.nil? && @control_id !~ Regexp.new(/^se(-[a-z0-9]+)+$/)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] control_id Value to be assigned
    def control_id=(control_id)
      if !control_id.nil? && control_id.to_s.length > 25
        fail ArgumentError, 'invalid value for "control_id", the character length must be smaller than or equal to 25.'
      end

      if !control_id.nil? && control_id.to_s.length < 1
        fail ArgumentError, 'invalid value for "control_id", the character length must be great than or equal to 1.'
      end

      if !control_id.nil? && control_id !~ Regexp.new(/^se(-[a-z0-9]+)+$/)
        fail ArgumentError, 'invalid value for "control_id", must conform to the pattern /^se(-[a-z0-9]+)+$/.'
      end
    ...
    def hash
      [invoice_date, invoice_number, control_id, invoice_amount].hash
    end
